...
export class TzTestComponent {
  ...
  // Returns a full ISO 8601 format
  // https://momentjs.com/docs/#/displaying/format/
  getDateTimeForZone(zone: string): string {
    return moment(this.sampleDateTime).tz(zone).format();
  }

  // Returns "-05:00" for a given `zone` (US/Eastern) and date
  getOffset(zone: string): string {
    // https://momentjs.com/timezone/docs/#/using-timezones/converting-to-zone/
    return moment(this.sampleDateTime).tz(zone).format('Z');
  }
