...
export class TzTestComponent {
  ...
  getLuxonForZone(zone: string): DateTime {
    const dateTime: DateTime = DateTime.fromObject(this.sampleDateTime, { 'zone': zone });
    const displayedDateTime = dateTime.setZone('America/New_York');
  ...
  getOffset(zone: string): string {
    const dateTime: DateTime = DateTime.fromObject(this.sampleDateTime, { 'zone': zone });
    const ianaZone: IANAZone = IANAZone.create(zone);

    return ianaZone.formatOffset(dateTime.toMillis(), "short");
  }

  // Returns "-300" for a given `zone` (US/Eastern) and date
  getOffsetNum(zone: string): number {
    const dateTime: DateTime = DateTime.fromObject(this.sampleDateTime, { 'zone': zone });
    const ianaZone: IANAZone = IANAZone.create(zone);

    return ianaZone.offset(dateTime.toMillis());
  }

  // Sorts this.zones by their offset
  getSortedZones() {
    return this.zones.sort((a, b) => {
      const offsetA: number = this.getOffsetNum(a);
      const offsetB: number = this.getOffsetNum(b);
